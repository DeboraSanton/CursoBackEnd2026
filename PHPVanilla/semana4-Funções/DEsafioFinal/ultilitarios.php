...
function gerarIniciais(string $nomeCompleto): string {
    ...
    //A função gerarIniciais() recebe o nome completo como parâmetro.
    //O `explode()` separa o nome em palavras. Depois o `foreach` percorre cada uma dessas palavras.
    //O `substr()` pega a primeira letra de cada palavra, já o `strtoupper()` transforma essa letra
    // Em maiusculo, essas letras são armazenadas na variavel `$iniciais` dai no fim o `return` retorna o resultado da função

$palavras = explode(" ", $nomeCompleto); // => [ana, Carolina, Silva]
// => percorre o vetor e pegar cada inicial das palavras
$iniciais = "";
foreach ($palavras as $palavras) {
    if($palavras !== ""){
        $iniciais .= substr($palavras, 0, 1); // pegar a primeira letra da palavra e concatenar na variavel $iniciais
        }
    }
