...
Antes do PHP processar qualque informação, precisamos coletar informações no frontend através de um `<form>`
...
<form action="processa.php" method="POST">
  ...
  <input type="text" id="compoNome" name="nomeUsuario" placeholder="Digite seu nome">
...
O método `GET` é ultilizado quando a intenção do cliente é **buscar ou filtrar dados** sem alterar o estado do servidor. Os dados enviados via `GET` são enexadas diretamente ao final da URL na forma de uma **Query String**.


2. O método POST (Envio de cargas úteis e mutações)
 
 O método `POST` é ultilizada quando o formulario envia dados que devem ser processados para **criar ou modificar registros** no sistema (ex: cadastro de usuario, finalização de compras, upload de arquivos)
...
- **$-GET**: Armazena dados passados pela URL via parâmetros de consultas (query string);
- **$_POST**: Recolhe dados por formúlarios usando métodos HTTP POST.
- **$_SERVER**: Contém informações sobre o sereviço, ambiente e caminhos de script.

**Porque usamos `??` para obter dados da SuperGlobal??**

Usamos o Operador de Nulidade (Coalescência Nula) para verificar os valores das variaveis não é `null`, se caso for `null` atribuimos um valor para evitar erros no script.
...
```php
$nome = $_POST["nome"];
// Se escrever desta forma, o código poode gerar um aviso de erro.

//A forma correta de escrita é
$nome = $_Post["nome"] ?? "";
// Se $_POST["nome"] não existir, use uma string vazia.

//Outra forma de verificar nilidade é usando if/else
if(isset($_POST["nome"])){
  $nome = $_POST["nome"];
}else{
  $nome = "";
...
Muitos desenvolvedores iniciantes acreditam que colocar atributos `required`, `typw="email"` ou `min=0` na <tag> do HTML é sulficiente para proteger o sistema. **isso é Ilusão**. Sempre devemos fazer validações de dados no código Backend. As validações no BackEnd devem acontecer sempre antes do processamento de qualquer dadorecebidopelo usuario.
...
>obs: use htmlspecialchars() ao exibir valor em HTML => converte caracteres especiais em entidades correspondentes em HTML, evitando que o codigo seja interpretado erradamente pelo navegador. é usado preincipalmente na segurança web, para evitar ataques Cross-Site-Scriptin(XSS).


#### Preservação de Estado em Formulários (*Stick Form*)

A técnica do **Sticky Form** consiste em imprimir de volta o valor no atributo "Values" do input, os dados que o usuário acaba de digitar, os valores são devolvidos aos inputs, caso ocorra algum erro de validade de dados no envio.
...
### Semana 7 - Segurança no BackEnd - Sanitização Validacão e Proteção contra XSS

**1 Mandamento do Desenvolvedor BackEnd**

> Nunca confie no Usuário: Toda entrada de dados vinda de fora do servidor é potencialmente maliciosa até que seja rigorosamente validado, sanitizada e codificada.

Quando você disponibiliza um campo de texto em um site, qualquer pessoa conectada a internet pode digitar códigos maliciosos em vez de texto. Se o código BackEnd pega esse texto diretamente sem nenhum tratamento, a ordem de excurção de código abrirá portas para a invasão devastadora do sistema.

**A Anatomia de um Ataque: O que é Cross-Site Scripting (XSS)**

O XSS ocorre quando uma aplicação web inclui dados não confiaveis em uma página web sem a devida validação ou escape de caracteres. Isso, permite que um atacante execute scripts maliciosos (geralmente em JvaScript) diretamente no navegador de outro usuario que visitam o site.
...
#### **A Tríagem de Defesa: Validação, Sanitização e Escapamento**

1. **Validação**: Verifica se o dado recebido atende aos reqisitos exatos do sistema (tipo, tamanho, formato).

Ex: vrificar se o E-mail possui `@` e dominio válido (`filer_var($email, FILTER_VALIDADE_EMAIL)`).

2. **Sanitização**: Transforma o dado para adequa-lo ao fromato desejado, removendo caracteres indesejados.

Ex:REmoveer espaços no início e fim (`trim($nome)`).

3. **Escapamento/Codificação de Saída**: é o ato de converter caracters especiais de linguagem HTML em suas respecivas **Entidades HTML** no momento exato em que eles são impressos na tela.

ex: usar `htmlspecialchar()`.
...
```php
string htmlspecialchar(
  string $string,
  int $flags = ENT_QUOTTES | ENT_SUBTITUTE |
  ENT_HTML5,
  ?string $encoding = "UTF-8"
)
```

- `ENT_QOUTES`: Converte tanto aspas duplas quanto aspas simples
- `EMT_SUBSTITUTE`: Substitui sequências de bytes inválidos por caracteres de substituição Unicode em vez de retornar uma string vazia
- `ENT_HTML5`: Aplica a tabela de entidade compatíveis com a especificação HTML5
- `UTF-8`: Garante que caracteres de lingua portuguesa com "ç", "ã", "é" sejam preservados sem corrupção.

**A Função helper de Escapamento**

Para não digitar essa linha externa em todas as partes de saída de texto para HTML, os desenvolvedores proficionais criam uma função auxiliar curta:
